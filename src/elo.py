...
class EloRatings:
    ...
    def update(self, home_team, away_team, home_goals, away_goals, neutral=False):
        """Update ratings after a match. Returns pre-match ratings (for feature use).

        neutral: True for matches played at a neutral venue (e.g. a UEFA final),
        where the home_team/away_team labels are just nominal and shouldn't get
        a home-advantage bump. Defaults to False, so existing single-competition
        callers (no neutral matches in their data) are unaffected.
        """
        ...
        exp_home = self.expected_score(home_rating + home_adv, away_rating)
        exp_away = 1 - exp_home
...
def compute_elo_features(df, k=20, home_advantage=60):
    ...
    If df has a `neutral` column (1/0 or True/False), rows flagged neutral
    skip the home-advantage bump (see EloRatings.update). Absent entirely
    for datasets that never have neutral-venue matches (e.g. epl_results.csv).
    """
    df = df.sort_values("date").reset_index(drop=True)
    elo = EloRatings(k=k, home_advantage=home_advantage)
    has_neutral_col = "neutral" in df.columns

    home_elos, away_elos = [], []
    for _, row in df.iterrows():
        neutral = bool(row["neutral"]) if has_neutral_col and pd.notna(row["neutral"]) else False
        pre_home, pre_away = elo.update(
            row["home_team"], row["away_team"], row["fthg"], row["ftag"], neutral=neutral
        )
