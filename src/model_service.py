...
_UNSET = object()  # sentinel: distinguishes "xg_path not passed" from "xg_path=None"


class ModelService:
    def __init__(self, results_path, odds_path=None, xg_path=_UNSET,
                 home_advantage=60, lookback_seasons=None, label=None):
        """
        odds_path: optional. Historical closing-odds CSV in the epl_odds.csv
            format (bet365_1x2_home, etc.) — only used by callers that want
            to merge it themselves (e.g. backtest.py); _train() below never
            reads it, so competitions with no odds history (e.g. UCL) can
            just pass None.
        xg_path: optional. If not given, falls back to <results_dir>/epl_xg.csv
            for backward compatibility with the original single-competition
            deploy; pass xg_path=None explicitly (not just omit it) for a
            competition that has no xG source at all — see competitions.py.
        home_advantage: Elo home-advantage constant, competition-specific
            (see competitions.py for why UCL uses a lower value than EPL).
        lookback_seasons: if set, only the most recent N distinct `season`
            values in results_path are used for training — see the UCL
            entry in competitions.py for why that's the right default there.
        """
        self.results_path = results_path
        self.odds_path = odds_path
        self.home_advantage = home_advantage
        self.lookback_seasons = lookback_seasons
        self.label = label or os.path.basename(results_path)
        # Auto-discovers epl_xg.csv next to results_path only when xg_path is
        # omitted entirely (not passed at all) — pass xg_path=None explicitly
        # to opt out (competitions.py does this for UCL).
        if xg_path is _UNSET:
            xg_path = os.path.join(os.path.dirname(results_path), "epl_xg.csv")
    ...
    def _load_data(self):
        ...
        if self.lookback_seasons and "season" in results.columns:
            recent_seasons = (
                results[["season"]].drop_duplicates()["season"]
                .pipe(lambda s: sorted(s, key=lambda season: str(season)))[-self.lookback_seasons:]
            )
            n_before = len(results)
            results = results[results["season"].isin(recent_seasons)].copy()
            logger.info(f"[{self.label}] lookback_seasons={self.lookback_seasons}: "
                        f"kept {len(results)}/{n_before} rows from seasons {sorted(recent_seasons)}")

        if not self.odds_path:
            return results, None
    ...
    def _train(self):
        logger.info(f"[{self.label}] Training model on startup...")
        results, odds = self._load_data()
        results = results.sort_values("date").reset_index(drop=True)

        results, elo_engine = compute_elo_features(results, home_advantage=self.home_advantage)
        results = add_rolling_form(results)
        results = add_rest_days(results)

        use_xg = bool(self.xg_path) and os.path.exists(self.xg_path)
        if use_xg:
            ...
            logger.info(f"[{self.label}] No xG file configured/found — serving with the goals-only "
                        f"feature set. (Run download_xg_data.py for EPL; no xG source is wired up "
                        f"for other competitions yet.)")
        self.use_xg = use_xg
        ...
        logger.info(f"[{self.label}] Model trained on {len(train_df)} matches. Ready.")
